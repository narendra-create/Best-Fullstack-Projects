...
const Navbar = ({ toggleDrawer }) => {
    ...
    const isDashboard = pathname === '/VendorTools/Dashboard'
    const bgcolor = isRegisterpage || isLoginpage || pathname === '/' ? "white" : "black";
    const dashboardcolor = isDashboard ? "border-black text-black hover:bg-black hover:text-white" : "text-white border-white  hover:bg-white hover:text-black"
    ...
    return (
        <nav className='w-full h-24 absolute z-40 top-0'>
            <div className='w-full md:px-10 px-6 mx-auto h-24 flex justify-between items-center'>
                ...
                {User && User.role === 'customer' ? <div className='flex md:gap-16 gap-3 items-center justify-center'>
                    <button
                    ...
                    {!isDashboard && <Link href={'/VendorTools/Dashboard'} className='font-semibold px-5 py-2 border-3 border-white text-white hover:bg-white hover:text-black rounded-2xl transition-all ease-in-out duration-200'>
                        DASHBOARD
                    </Link>}
                </div>}
            </div>
