            ...
            {isLoading ? <div>.......</div> :
                <div className='w-full md:px-10 px-6 mx-auto h-24 flex justify-between items-center'>
                    ...
                    {(iscustomer || isguest) ? <div className='flex md:gap-16 gap-3 items-center justify-center'>
                        ...
                        {!isDashboard && User && User.role === 'vendor' && <Link href={'/VendorTools/Dashboard'} className='font-semibold px-5 py-2 border-3 border-white text-white hover:bg-white hover:text-black rounded-2xl transition-all ease-in-out duration-200'>
                            DASHBOARD
                        </Link>}
                    </div>}
                </div>
